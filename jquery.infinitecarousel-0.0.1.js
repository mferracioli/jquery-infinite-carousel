(function ($) {
    $.fn.infiniteCarousel = function (options) {
        ...
        $("<ul class=\"pager\"></ul>").appendTo(this);
        var pager = $("ul.pager", $(this));

        $("ul li", this).each(function (i, li) {
            $(li).css("left", (i * options.width));
            $(pager).append("<li><a class=\"page-" + i + "\" href=\"#\">" + (i + 1) + "</a></li>");
            $("a:eq(" + i + ")", $(pager)).click(function () {
                jumpTo(i);
            });
        });

        var index = 0;
        var count = $("a", $(pager)).length;
        mark();
        ...
        function mark() {
            ...
            $("li:eq(" + index + ")", $(pager)).addClass("active");
        }
